...
def optimize_file(source_file_path, verbose=False):
    ...
    # If optimized pdf is smaller than source, create compressed pdf with a new name
    if output_file_size < source_file_size:
        # Create compressed pdf with a new name
        compressed_file_name = source_file_name.replace(".pdf", "_compressed.pdf")
        output_file_path_compressed = os.path.join(source_file_dir, compressed_file_name)

        # Rename optimized pdf
        os.rename(output_file_path, output_file_path_compressed)
